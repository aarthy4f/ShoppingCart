...
function execute(data) {
    ...
    prodSelection.selectSize(data.size);
        ...
        prodSelection.validateselection(data.sizeverify,data.quanity)
         ...
         if(data.addquanity>1){
             ...
             checkoutvalue=checkoutvalue*data.addquanity
         }
  ...
  /*beforeEach(() => {
    ...
    // so we must tell it to visit our website with the `cy.visit()` command.
    // Since we want to visit the same URL at the start of all our tests,
    // we include it in our beforeEach function so that it runs befor*e each test
    cy.visit('https://react-shopping-cart-67954.firebaseapp.com/')*/
  //})

  /*it('displays two todo items by default', () => {
   
   /* prodSelection.selectSize("small");
   // cy.xpath('//span[text()="S"]').click().wait(6000)
    //cy.xpath('//div[data-sku="12064273040195392"]//following::div[@class="shelf-item__buy-btn"]').first().should("be.visible").click({force:true})

      //  cy.xpath('//div[@class="shelf-item__buy-btn"]').first().should("be.visible").click({force:true})
        //cy.xpath('//p[@class="desc"]').contains('S').contains("Quantity: 1")
           
        prodSelection.validateselection("S","1")

        prodSelection.completecheckout("10.90");
     /*   cy.xpath('//p[@class="desc"]')
         .invoke('text')
  .then((text1) => {
      expect(text1).to.contains('S')
      expect(text1).to.contains('Quantity: 1')
  })*/
  /*cy.xpath('//div[@class="buy-btn"]').should("be.visible").click({force:true})
